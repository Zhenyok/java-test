...
(function(angular){
    ...
    .controller('JcropController', ['$scope', '$element', 'ngJcropConfig','$timeout',
    function($scope, $element, ngJcropConfig, $timeout){
        ...
        $scope.showPreview = function(coords){
            if( !$scope.selectionWatcher ){
                    if(parseInt(coords.w, 10) == 0  && parseInt(coords.h, 10) == 0) {
                        $scope.$emit("selectedDisable","off");
                    } else {
                        $scope.$emit("selectedDisable","on");
                    }
                    $timeout(function(){
                        angular.element('.jcrop-keymgr+div').show();
        ...
        $scope.onMainImageLoad = function(){
            ...
            var im = new Image();
            im.src = $scope.mainImg[0].src;
            if (!angular.isUndefined(im) && im.width > 0 && im.height > 0) {
                ngJcropConfig.jcrop.trueSize=[im.width,im.height];
            }
        ...
        $scope.$on("reSelected",function(evt, data){
            if (angular.isArray(data)) {
                $timeout(function(){
                    $scope.jcrop.animateTo([data[0], data[1],data[2],data[3]]);
                });
                var coordinate = {
                    x:Math.round(data[0]),
                    y:Math.round(data[1]),
                    x2:Math.round(data[2]),
                    y2:Math.round(data[3]),
                    w:Math.round(data[4]),
                    h:Math.round(data[5])
                };
                $scope.showPreview(coordinate);
            }
        });
        ...
        $scope.$on("jcropRelease", function(event, data){
            $scope.jcrop.release();
            angular.element('.jcrop-keymgr+div').hide();
        });
